...
public class ProductRepositoryImpl implements IProductRepository {
    ...
    private static final String INSERT_PRODUCTS_SQL = "INSERT INTO products" + "  (id_category, name, price, quantity, color, description) VALUES " +
            " (?, ?, ?, ?, ?, ?);";
    private static final String SELECT_ALL_PRODUCTS = "select * from products p inner join category c on p.id_category = c.id_category order by p.id desc;";
    ...
    private static final String SEARCH_BY_NAME = "select * from products p inner join category c on p.id_category = c.id_category where p.name LIKE concat('%',?,'%') order by p.id desc;";
    private static final String SEARCH_BY_PRICE = "select * from products p inner join category c on p.id_category = c.id_category  where p.price LIKE concat('%',?,'%') order by p.id desc;";
    private static final String SEARCH_BY_QUANTITY = "select * from products p inner join category c on p.id_category = c.id_category  where p.quantity LIKE concat('%',?,'%') order by p.id desc;";
    private static final String SEARCH_BY_CATEGORY = "select * from products p inner join category c on p.id_category = c.id_category where c.name_category LIKE concat('%',?,'%') order by p.id desc;";
